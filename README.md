...
主循环：
（1）是否发现敌人，如果检测到敌装甲，则停止云台扫描；否则开启云台扫描；两种情况下底盘都保持旋转 0.5。
（2）进入进攻分支前置条件：自身状态良好（血量≥200、热量≤150），否则进入撤退分支。
（3）进入进攻分支，下发控制区 OccupyArea 目标
（4）否则转撤退分支，下发补给区 SupplyArea目标。
（5）若判定“被攻击”，先执行闪避（获取当前位置→在附近生成随机点→导航过去，循环 3 次），否则进入下一层逻辑。
（6）若自身状态再次满足“血量≥200、热量≤150”，进入进攻分支
